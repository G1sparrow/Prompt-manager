...
### 🔍 PNG 图片反推
上传 Stable Diffusion WebUI 或 ComfyUI 生成的 PNG/JPEG/WebP 图片，自动提取其中的提示词、反向提示词和生成参数（采样器、步数、CFG、种子等）。仅读取元数据，不存文件。支持一键将提取的提示词发送到扩写功能。保存时自动上传图片。
...
### 🗃️ SD 模型管理
管理 Stable Diffusion 模型列表，支持模型名称、推荐参数（采样器/CFG/VAE/Text Encoder）和说明，以卡片瀑布流展示。新建提示词时可通过下拉菜单选择已保存的模型（支持自定义输入）。
...
```bash
# 克隆仓库
git clone https://github.com/G1sparrow/Prompt-manager.git
cd promptmanager
